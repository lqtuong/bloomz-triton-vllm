...
class TritonPythonModel:
    def initialize(self, args):
        ...
        output_config = pb_utils.get_output_config_by_name(self.model_config, "TRANSLATED")
    ...
    def create_response(self, vllm_output):
        ...
        triton_output_tensor = pb_utils.Tensor(
            "TRANSLATED", np.asarray(text_outputs, dtype=self.output_dtype)
        )
    ...
    async def generate(self, request):
        ...
        try:
            ...
            async for output in self.llm_engine.generate(
                text, sampling_params, request_id
            ):
                if stream:
            ...
            triton_output_tensor = pb_utils.Tensor(
                "TRANSLATED", np.asarray(["N/A"], dtype=self.output_dtype)
            )
